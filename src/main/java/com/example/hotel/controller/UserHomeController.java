...
public class UserHomeController {
	 ...
	 HttpSession session;
	    ...
	    @GetMapping("/userHome/mycart")
	    public String cart(Model model) {
	    	
	    	User user = (User) session.getAttribute("user");
	    	 model.addAttribute("user", user);
	    	 
	      	List<Orders> listitem =	orderservice.listAllByTodayDateByUser(user);
	      	float sum = 0;
	    ...
	    @GetMapping("/userHome/history")
	    public String history(Model model) {
	    	User user = (User) session.getAttribute("user");
	    	 model.addAttribute("user", user);
	    	 
	      	List<Orders> listitem =	orderservice.listAllBy30DaysByUser(user);
	      	float sum = 0;
	    	for(int i = 0; i< listitem.size(); i++) {;
	    		Orders o = listitem.get(i);
	    		System.out.println(o.getItemname());
